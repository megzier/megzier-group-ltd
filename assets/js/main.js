...
Template Name: Megzier Group Ltd Website
Author: Megzier Group Ltd
Author URI: https://megzier.com
Version: 1.0.0
Description: Megzier Group Ltd marketing, software, web, and IT website
...
(function ($) {
	...
	function normalizePrimaryNavigation() {
		var pathName = window.location.pathname.split("/").pop().toLowerCase();
		...
		var homePages = [
			{ label: "IT Solutions", href: "index.html", active: isCurrent("index.html") },
			{ label: "Digital Marketing", href: "digital-marketing.html", active: isCurrent("digital-marketing.html") },
			{ label: "It Services", href: "it-services.html", active: isCurrent("it-services.html") },
			{ label: "Business Consulting", href: "business-consulting.html", active: isCurrent("business-consulting.html") },
			{ label: "Startup", href: "startup.html", active: isCurrent("startup.html") },
			{ label: "Software Agency", href: "software-agency.html", active: isCurrent("software-agency.html") },
			{ label: "Web Design Agency", href: "web-design-agency.html", active: isCurrent("web-design-agency.html") },
			{ label: "AI Agency", href: "ai-agency.html", active: isCurrent("ai-agency.html") },
			{ label: "Sass", href: "sass.html", active: isCurrent("sass.html") }
		];

		var blogPages = [
			{ label: "Blog Grid", href: "blog-grid.html", active: isCurrent("blog-grid.html") },
			{ label: "Blog List", href: "blog-list.html", active: isCurrent("blog-list.html") },
			{ label: "Blog Details", href: "blog-details.html", active: isCurrent("blog-details.html") }
		];

		var homeActive = homePages.some(function (item) { return item.active; });
		var aboutActive = isCurrent("about.html");
		var servicesActive = isCurrent("service.html") || isCurrent("service-details.html");
		var portfolioActive = isCurrent("project.html") || isCurrent("project-details.html");
		var pricingActive = isCurrent("pricing.html");
		var blogActive = blogPages.some(function (item) { return item.active; }) || isCurrent("blog.html");
		var contactActive = isCurrent("contact.html");

		var navHtml = '';
		navHtml += '<li class="menu-item-has-children' + (homeActive ? ' active' : '') + '">';
		navHtml += '<a class="' + (homeActive ? 'active' : '') + '" href="#">Home</a>';
		navHtml += '<ul class="sub-menu">';
		homePages.forEach(function (item) {
			navHtml += '<li' + (item.active ? ' class="active"' : '') + '><a href="' + item.href + '"' + (item.active ? ' class="active"' : '') + '>' + item.label + '</a></li>';
		});
		navHtml += '</ul>';
		navHtml += '</li>';
		navHtml += '<li' + (aboutActive ? ' class="active"' : '') + '><a' + (aboutActive ? ' class="active"' : '') + ' href="about.html">About</a></li>';
		navHtml += '<li' + (servicesActive ? ' class="active"' : '') + '><a' + (servicesActive ? ' class="active"' : '') + ' href="service.html">Services</a></li>';
		navHtml += '<li' + (portfolioActive ? ' class="active"' : '') + '><a' + (portfolioActive ? ' class="active"' : '') + ' href="project.html">Portfolio</a></li>';
		navHtml += '<li' + (pricingActive ? ' class="active"' : '') + '><a' + (pricingActive ? ' class="active"' : '') + ' href="pricing.html">Pricing</a></li>';
		navHtml += '<li class="menu-item-has-children' + (blogActive ? ' active' : '') + '">';
		navHtml += '<a class="' + (blogActive ? 'active' : '') + '" href="#">Blog</a>';
		navHtml += '<ul class="sub-menu">';
		blogPages.forEach(function (item) {
			navHtml += '<li' + (item.active ? ' class="active"' : '') + '><a href="' + item.href + '"' + (item.active ? ' class="active"' : '') + '>' + item.label + '</a></li>';
		});
		navHtml += '</ul>';
		navHtml += '</li>';
		navHtml += '<li' + (contactActive ? ' class="active"' : '') + '><a' + (contactActive ? ' class="active"' : '') + ' href="contact.html">Contact</a></li>';

		$(".tv-header .main-menu .navigation").each(function () {
			$(this).html(navHtml);
		});
	}
	...
	=============================================*/
	function ajaxForm() {
		const whatsappNumber = "254768651556";
		const serviceOptions = `
			...
			<option value="custom-software">Custom Software & POS</option>
			<option value="ecommerce-solutions">E-Commerce Solutions</option>
			<option value="branding">Graphic Design & Branding</option>
			<option value="gmb-seo">Google My Business & SEO</option>
			<option value="cloud-email">Professional Email & Cloud</option>
			<option value="portfolio-profile">Portfolio & Profile Creation</option>
			<option value="surveys-data">Surveys & Data Collection</option>
			<option value="network-infrastructure">Network & IT Infrastructure</option>
			<option value="data-analytics">Data & Analytics</option>
			<option value="research-feasibility">Research & Feasibility</option>
			<option value="cloud-hosting">Cloud Services & Hosting</option>
			<option value="training-capacity">IT Training & Capacity Building</option>
			<option value="managed-support">Managed IT Support</option>
			<option value="digital-payments">Digital Payments Integration</option>
		`;
		...
		const standardizeContactForms = () => {
			document.querySelectorAll(".contact_form").forEach(form => {
				form.action = "#";

				const existingPhone = form.querySelector('[name="phone"]');
				const existingService = form.querySelector('[name="service"]');
				const messageField = form.querySelector('[name="msg"]');
				const messageGroup = messageField ? messageField.closest(".form-group") : null;
				const phoneValue = existingPhone ? existingPhone.value : "";
				const phoneGroup = existingPhone ? existingPhone.closest(".form-group") : null;
				const serviceValue = existingService ? existingService.value : "";
				const serviceGroup = existingService ? existingService.closest(".form-group") : null;
				...
				contactGrid.innerHTML = `
					<div class="form-group">
						...
						<input type="text" id="${form.id || "contact_form"}-phone" name="phone" placeholder="Phone No." required autocomplete="on">
					</div>
					<div class="form-group">
						<span class="icon"><i class="fa-solid fa-briefcase"></i></span>
						<select class="custom-select" id="${form.id || "contact_form"}-service" name="service" required autocomplete="off">
							${serviceOptions}
						</select>
					</div>
				`;

				if (messageGroup && messageGroup.parentNode === form) {
					form.insertBefore(contactGrid, messageGroup);
				} else if (messageGroup && messageGroup.parentNode) {
					messageGroup.parentNode.insertBefore(contactGrid, messageGroup);
				} else {
					form.appendChild(contactGrid);
				}

				const phoneField = contactGrid.querySelector('[name="phone"]');
				const serviceField = contactGrid.querySelector('[name="service"]');
		...
		standardizeContactForms();

		const openWhatsApp = form => {
			const serviceField = form.querySelector('[name="service"]');
			const serviceValue = getSelectedText(serviceField) || getFieldValue(form, '[name="service"]');
			const messageParts = [
				"Megzier Group Ltd inquiry",
				`Name: ${getFieldValue(form, '[name="name"]')}`,
				`Email: ${getFieldValue(form, '[name="email"]')}`,
				`Phone: ${getFieldValue(form, '[name="phone"]')}`,
				`Service: ${serviceValue}`,
				`Message: ${getFieldValue(form, '[name="msg"]')}`,
			].filter(line => !line.endsWith(": "));

			const whatsappUrl = `https://wa.me/${whatsappNumber}?text=${encodeURIComponent(messageParts.join("\n"))}`;
		...
		const handleFormSubmission = form => {
			form.addEventListener("submit", ev => {
				...
				if (form.classList.contains('contact_form')) {
					...
					openWhatsApp(form);
					showPopup('success', 'Opening WhatsApp with your details...');
					form.reset();
					return;
				}
				...
				sendAjaxRequest("POST", form.action, data,
					() => handleSuccess(form, submitButton),
					(statusCode, responseText) => handleError(statusCode, responseText, form, submitButton)
				);
			});
		};

		const handleSuccess = (form, submitButton) => {
			form.reset();
			let message = 'Success!';
		...
		const handleError = (statusCode, responseText, form, submitButton) => {
